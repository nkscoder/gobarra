...
class Spreadsheet_Excel_Reader {
    ...
    function Spreadsheet_Excel_Reader(){
	   ...
	   //$this->setUTFEncoder('utf-8'); // Added by Anand
    ...
    */
    function setUTFEncoder($encoder = 'iconv'){
		...
		/*elseif ($encoder == 'utf-8'){
        	$this->_encoderFunction = function_exists('utf8_encode') ? 'utf-8' : '';
		}*/
    }
    ...
    function read($sFileName) {
    	...
    	//error_reporting($errlevel ^ E_NOTICE ^ E_WARNING);
		error_reporting(0);
        ...
        if($res === false) {
        	...
        	// bad file
        		#die('Please save the file and then upload.');
				echo "<script>alert('Please Save the File and then upload.');window.history.go(-1);</script>";
				exit;
        	}
    ...
    function _encodeUTF16($string){
        ...
        if ($this->_defaultEncoding){
        	switch ($this->_encoderFunction){
				...
				case 'utf-8' : 	$result = mb_convert_encoding($string, $this->_defaultEncoding, 'UTF-16LE' );
        						break;
        	}
        }
        return $result;
