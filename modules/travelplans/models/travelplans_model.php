...
class travelplans_model extends MY_Model{
 ...
 public function getUserID($email){
	...
	public function get_countryAndCity($city)
	{
	$this->db->select('country.*,city.*');
	$this->db->from('tbl_country as country');
	$this->db->join('tbl_cities as city','country.country_id = city.country_id','left');
	...
	{
	$this->db->select('plan.travel_from,plan.travel_to,plan.user_id,plan.from_date,plan.to_date');
	$this->db->from('tbl_travel_plans as plan');
	$this->db->where('plan.user_id',$userid);
	$this->db->limit(5);
	$this->db->order_by('plan.travel_id','desc');
	$query = $this->db->get();
	if($query->num_rows()>0)
	{
			foreach($query->result() as $row)
