	...
	public function __construct()
	{
	 ...
	 public  function index($page = NULL)
	 {
		...
		public function add()
		{
			...
			$this->form_validation->set_rules('page_url','URL',"trim|required|unique[wl_meta_tags.page_url='".$url_posted."'] ");
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				'meta_added_date'=>$this->config->item('Year: %Y Month: %m Day: %d - %h:%i %a'),				
				);
	   ...
	   public function edit()
	   {
		 ...
		 { 
			...
			$this->form_validation->set_rules('page_url','URL',"trim|required|xss_clean|unique[wl_meta_tags.page_url='".$url_posted."' AND meta_id!='".$this->db->escape_str($res['meta_id'])."']");
