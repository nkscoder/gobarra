...
class Country extends Admin_Controller
{
	...
	public function __construct(){
		    ...
		    $this->config->set_item('menu_highlight','Data Management');	
	   ...
	   {		 
		...
		{				
			...
			$this->form_validation->set_rules('country_id','Country Id',"trim|required|max_length[50]|xss_clean|unique[tbl_country.country_id='".$this->db->escape_str($this->input->post('country_id'))."' AND status!='2']");
		...
		if($this->input->post('action')=='excel_file')
		{
			...
			if($this->form_validation->run()==TRUE)
			{
				...
				if(is_array($worksheet) && !empty($worksheet))
				{
					...
					{
										$country_name		=	(!isset($worksheet[$i][1])) ? '' : addslashes(trim($worksheet[$i][1]));
