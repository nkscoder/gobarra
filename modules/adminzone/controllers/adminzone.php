<?php
class adminzone extends Public_Controller {
	...
	public  function index()
	{
		...
		{
		  redirect('adminzone/dashbord/', 'refresh');
		}
		else
		{
	...
	{	
		...
		{	
			...
			{				
				$this->adminzone_model->check_admin_login( $postdata );
	...
	private function forgot_password_mail($email)
	{
		...
		$res_data =  $this->db->get_where('tbl_admin', array('admin_email' =>$email,'admin_id'=>'1' ))->row();
		...
		{ 
			...
			$verify_url= "<a href=".base_url()."adminzone/>Click here </a>";
			...
			redirect('adminzone/forgotten_password', '');
